...
# Select only the body and surface columns for clustering
data_to_cluster <- data[, c("body", "surface")]

# Use the Elbow method to determine the optimal number of clusters
set.seed(42)  # for reproducibility
wss <- sapply(1:10, function(k){kmeans(data_to_cluster, k, nstart = 10 )$tot.withinss})

# Plot the Elbow method results
elbow_plot <- data.frame(Number_of_Clusters = 1:10, WSS = wss)
ggplot(elbow_plot, aes(x = Number_of_Clusters, y = WSS)) + 
  geom_point() +
  geom_line() +
  theme_minimal() +
  ggtitle("Elbow Method for Determining Optimal Number of Clusters")

# Perform K-means clustering with the chosen number of clusters (assuming 2 from the Elbow plot)
set.seed(42)
kmeans_result <- kmeans(data_to_cluster, centers = 2)

# Attach cluster results to the original data
data$cluster <- as.factor(kmeans_result$cluster)

# Plot the clusters
cluster_plot <- ggplot(data, aes(x = body, y = surface, color = cluster)) + 
  geom_point(alpha = 0.6, size = 4) +
  theme_minimal() +
  ggtitle("Cluster Plot of Seismic Data")
